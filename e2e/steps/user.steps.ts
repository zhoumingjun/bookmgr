...
When('我访问用户管理页面', async ({ page }) => {
  ...
  await page.locator('header').getByText(/用户管理|Users/i).click();
  await page.waitForLoadState('networkidle');
});

When('我点击用户 {string} 的编辑按钮', async ({ page }, username: string) => {
  const row = page.locator('tr').filter({ has: page.locator('td').first().filter({ hasText: new RegExp(`^${username}$`) }) });
...
When('我修改角色为 {string}', async ({ page }, role: string) => {
  ...
  // Click the dropdown option (not the already selected value)
  await page.locator('.ant-select-dropdown .ant-select-item-option-content').filter({ hasText: role }).click();
});
...
Then('我应该看到管理员角色标签', async ({ page }) => {
  await expect(page.locator('.ant-tag').filter({ hasText: /管理员|Admin/ }).first()).toBeVisible();
});

Then('我应该看到用户更新成功的提示', async ({ page }) => {
  await expect(page.locator('.ant-message')).toContainText(/用户已更新|User updated/i, { timeout: 10_000 });
});

Then('我应该看到用户删除成功的提示', async ({ page }) => {
  await expect(page.locator('.ant-message')).toContainText(/用户已删除|User deleted/i, { timeout: 10_000 });
});
